...
const TopNavigation = () => {
  ...
  return (
    <div className="sticky top-0 z-40 bg-gradient-to-b from-juricast-background to-juricast-background/95 backdrop-blur-md border-b border-juricast-card/20 py-2">
      <div className="flex flex-col">
        ...
        <div className="flex justify-between items-center px-4 py-2">
          ...
          <div className="flex items-center gap-2 md:gap-4">
              ...
              {isSearchActive && (
                ...
                >
                  <input 
                    ...
                    className="w-full py-2 px-4 pr-10 rounded-full bg-juricast-card/50 border border-juricast-card/30 text-sm focus:outline-none focus:ring-2 focus:ring-juricast-accent/50 focus:border-juricast-accent" 
            ...
            <motion.button 
              className="p-2 rounded-full hover:bg-juricast-card/50 transition-colors" 
        ...
        {/* Categorized Areas Navigation */}
        <CategorizedAreasNav areas={areas} />
      </div>
    </div>
  );
};
...
const MainNavigation = ({ path }: { path: string }) => {
  ...
  return (
    <div className="flex justify-center mb-3">
      <motion.div 
        className="bg-juricast-card/40 backdrop-blur-md py-2 px-4 rounded-2xl shadow-lg border border-juricast-card/30 flex justify-around items-center gap-1" 
...
const CategorizedAreasNav = ({ areas }: { areas: AreaCard[] }) => {
  const location = useLocation();
  const path = location.pathname;
  ...
  return (
    <div className="px-4">
      ...
      {juridicoAreas.length > 0 && (
        <div className="mb-3">
          <div className="flex items-center gap-2 mb-2 px-2">
            <BookOpen size={16} className="text-juricast-accent" />
            <h3 className="text-sm font-semibold text-juricast-text">Direito</h3>
          </div>
              ...
              {juridicoAreas.map(area => {
                ...
                return (
                  <Link 
                    ...
                    className={cn(
                      "px-3 py-2 rounded-full text-sm whitespace-nowrap transition-all duration-300 border",
                      isActive 
                        ? "bg-juricast-accent text-white border-juricast-accent shadow-lg" 
                        : "bg-juricast-card/30 text-juricast-text border-juricast-card/30 hover:bg-juricast-card/50 hover:border-juricast-accent/30"
                    )}
                  >
                    {area.name}
                    {area.episodeCount && (
                      <span className="ml-1 text-xs opacity-70">({area.episodeCount})</span>
                    )}
                  </Link>
                );
              })}
      ...
      {educativoAreas.length > 0 && (
        <div className="mb-2">
          <div className="flex items-center gap-2 mb-2 px-2">
            <GraduationCap size={16} className="text-juricast-accent" />
            <h3 className="text-sm font-semibold text-juricast-text">Educação Jurídica</h3>
          </div>
          <div className="overflow-x-auto scrollbar-hide">
            <div className="flex gap-2 pb-2 min-w-max">
