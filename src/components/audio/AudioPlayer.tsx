...
import ExpandableDescription from '@/components/podcast/ExpandableDescription';
...
const AudioPlayer: React.FC<AudioPlayerProps> = ({
    ...
    if (isPlaying) {
      visualizerIntervalRef.current = window.setInterval(() => {
        const newBars = Array(32).fill(0).map((_, index) => {
          ...
          const frequencyMultiplier = 1 - (index / 32) * 0.6;
      ...
      const fadeOut = () => {
        setVisualizerBars(prev => 
          prev.map(bar => Math.max(0, bar * 0.85))
        );
      };
  ...
  return (
    ...
    >
      {/* Main Audio Player */}
      <div className="p-4 md:p-6 bg-gradient-to-br from-juricast-card via-juricast-card to-juricast-card/80 rounded-xl border border-juricast-card/50 shadow-2xl relative overflow-hidden">
        {/* Background elements */}
        ...
        <div className="relative z-10">
          <div className="flex flex-col lg:flex-row items-center gap-6 mb-6">
            {/* Enhanced thumbnail */}
            <motion.div 
              className="relative w-48 h-48 md:w-56 md:h-56 lg:w-64 lg:h-64 flex-shrink-0 overflow-hidden rounded-2xl shadow-2xl" 
            ...
            >
              ...
              <motion.div 
                ...
                animate={{ opacity: isPlaying ? 0.8 : 1 }}
              >
                <motion.div
            ...
            <div className="flex-1 w-full">
              ...
              <div className="flex items-center gap-4 mb-4">
                <motion.button 
                  onClick={toggleMute} 
                  className="text-juricast-muted hover:text-juricast-text transition-colors"
                  whileHover={{ scale: 1.1 }}
                  whileTap={{ scale: 0.9 }}
                >
      ...
      {description && (
        ...
        >
          <ExpandableDescription 
            ...
            title="Mostrar Conteúdo do Episódio"
          />
        </motion.div>
      )}
    </motion.div>
