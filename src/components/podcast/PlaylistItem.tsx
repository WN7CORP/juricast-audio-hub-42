    ...
    scale: 1.01
      ...
      <Link to={`/podcast/${episode.id}`} className="flex items-center p-3">
        ...
        <div className="h-12 w-12 relative rounded-md overflow-hidden flex-shrink-0">
          <img src={episode.imagem_miniatura} alt={episode.titulo} className="h-full w-full object-cover" />
          <div className="absolute inset-0 bg-black/30 opacity-0 hover:opacity-100 flex items-center justify-center transition-opacity">
            <motion.button whileHover={{
            scale: 1.2
          }} whileTap={{
            scale: 0.9
          }} className="p-2 rounded-full bg-juricast-accent/90" onClick={handlePlay}>
        ...
        <div className="flex-1 ml-4 mr-2">
          ...
          {episode.progresso && episode.progresso > 0 && episode.progresso < 100 && <div className="mt-1 w-full h-1 bg-juricast-background rounded-full overflow-hidden">
