...
const SupportContent: React.FC<SupportContentProps> = ({ description, episode }) => {
              ...
              <div className="prose prose-sm max-w-none text-white prose-headings:text-white prose-strong:text-white prose-em:text-juricast-accent prose-p:text-white prose-ul:text-white prose-li:text-white">
                <ReactMarkdown 
                  components={{
                    ...
                    em: ({ children }) => <em className="text-juricast-accent">{children}</em>
                  }}
                >
                  {description}
                </ReactMarkdown>
                  ...
                  {Array.isArray(episode.tag) && episode.tag.length > 0 && (
                    <div className="mb-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {episode.tag.map((tag: string, index: number) => (
                          <span
                            key={index}
                            className="bg-juricast-background/50 px-2 py-1 rounded-full text-xs border border-juricast-card/30 text-white"
                          >
                            {tag}
                          </span>
                        ))}
