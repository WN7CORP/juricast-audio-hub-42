  ...
  return (
    <MainLayout>
      <motion.div
        ...
        className="space-y-8 sm:space-y-10 sm:px-0 px-0 py-0"
      >
        ...
        {inProgressEpisodes.length > 0 && (
          <motion.section variants={itemVariants}>
            ...
            <motion.div className="space-y-3" variants={containerVariants}>
              {loadingInProgress
                ? [...Array(3)].map((_, i) => (
                    <div key={i} className="bg-juricast-card animate-pulse rounded-lg h-16" />
                  ))
                : inProgressEpisodes.slice(0, 5).map((episode, index) => (
        ...
        {juridicoAreas.length > 0 && (
          <motion.section id="areas-direito" variants={itemVariants}>
            ...
            <motion.div 
              ...
              variants={containerVariants}
            >
              {juridicoAreas.slice(0, 8).map((area) => (
  ...
  return (
    <motion.div className="overflow-x-auto pb-4" variants={containerVariants}>
      <div className="flex space-x-4 min-w-max">
        {isLoading
          ? [...Array(4)].map((_, i) => (
              <div key={i} className="bg-juricast-card animate-pulse rounded-lg h-40 sm:h-48 w-48 sm:w-64" />
            ))
          : recentEpisodes.slice(0, 5).map((episode) => (
              <motion.div key={episode.id} className="flex-shrink-0 w-48 sm:w-64" variants={itemVariants}>
                <Link 
