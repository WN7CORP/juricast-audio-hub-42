  ...
  return <MainLayout>
      <motion.div className="space-y-10" initial="hidden" animate="visible" variants={containerVariants}>
        ...
        {juridicoAreas.length > 0 && <motion.section id="areas-direito" variants={itemVariants}>
            <motion.div className="flex justify-between items-center mb-6" variants={sectionHeaderVariants}>
...
const NewEpisodesCarousel = () => {
  ...
  return <motion.div className="overflow-x-auto pb-4" variants={containerVariants}>
      <div className="flex space-x-4 min-w-max">
        {isLoading ? [...Array(4)].map((_, i) => <div key={i} className="bg-juricast-card animate-pulse rounded-lg h-48 w-64"></div>) : recentEpisodes.slice(0, 5).map(episode => <motion.div key={episode.id} className="flex-shrink-0 w-64" variants={itemVariants}>
  ...
  return <motion.section variants={{
    ...
    visible: {
      ...
      visible: {
        ...
        {isLoading ? [...Array(5)].map((_, i) => <div key={i} className="bg-juricast-card animate-pulse rounded-lg h-16"></div>) : recentEpisodes.slice(0, 5).map((episode, index) => <PlaylistItem key={episode.id} episode={episode} index={index + 1} />)}
      </motion.div>
    </motion.section>;
};
