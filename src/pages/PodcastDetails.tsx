      ...
      <motion.div initial="hidden" animate="visible" variants={fadeIn}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            ...
            duration: 0.4
              ...
              <div className="flex border-b border-juricast-background mb-6">
                <button className={cn("px-6 py-3 border-b-2 transition-colors font-medium", activeTab === 'visualizacao' ? "border-juricast-accent text-juricast-accent" : "border-transparent text-juricast-muted")} onClick={() => setActiveTab('visualizacao')}>
                  Visualização
                </button>
                <button className={cn("px-6 py-3 border-b-2 transition-colors font-medium", activeTab === 'detalhes' ? "border-juricast-accent text-juricast-accent" : "border-transparent text-juricast-muted")} onClick={() => setActiveTab('detalhes')}>
                  Detalhes
                </button>
              </div>
          ...
          <div className="lg:col-span-1">
            ...
            <motion.div className="mt-6 bg-juricast-card rounded-lg p-4 border border-juricast-card/30" initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
            ...
            delay: 0.3
              ...
              <div className="bg-gradient-to-r from-juricast-card to-juricast-card/70 p-4 rounded-lg border border-juricast-card/30">
                ...
                <Link to={`/categoria/${episode.area.toLowerCase().replace(/\s+/g, '-')}`} className="block text-center py-2 px-4 bg-juricast-accent text-white rounded-md hover:bg-juricast-accent/80 transition-colors">
                  Ver todos
                </Link>
              </div>
            </motion.div>
...
const AnimatedTabContent: React.FC<AnimatedTabContentProps> = ({
  ...
  return <motion.div key={activeTab} initial={{
    opacity: 0
  }} animate={{
    opacity: 1
  }} exit={{
    ...
    duration: 0.3
      ...
      {activeTab === 'visualizacao' && <div>
          
          
        </div>}

      {activeTab === 'detalhes' && <div>
          <motion.p className="mb-4" initial={{
        opacity: 0,
        y: 10
      }} animate={{
        opacity: 1,
